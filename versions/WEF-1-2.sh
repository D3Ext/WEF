...
function ctrl_c2(){
	...
	rm *.tmp 2>/dev/null
	rm rainbowtables 2>/dev/null
	rm wef-wordlist.txt 2>/dev/null
	rm pkmid_hashes 2>/dev/null
	rm pkmid_capture 2>/dev/null
	sleep 0.2
	tput cnorm
	exit 0
}
...
function handshake_crack(){
	...
	if [ "$(echo $?)" == "0" ]; then
		...
		rainbow-gen
		sleep 2.5
		aircrack-ng -r ${dir}/rainbowtables Capture-01.cap
		sleep 0.3
...
function rainbow-gen(){
	...
	head -n 1000000 ${wordlist_to_use} > wef-wordlist.txt 2>/dev/null
	echo -e "${name}" > essid.tmp 2>/dev/null
	airolib-ng rainbowtables --import passwd wef-wordlist.txt &>/dev/null
	airolib-ng rainbowtables --import essid essid.tmp &>/dev/null
	timeout 90 bash -c "airolib-ng rainbowtables --batch"
